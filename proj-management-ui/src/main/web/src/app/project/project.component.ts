...
export class ProjectComponent implements OnInit {
  ...
  ngOnInit() {
    this.projectService.fetchAllProjs().subscribe(projectDtos => {
  ...
  openSubModel(content) {
    this.modalService.open(content, {ariaLabelledBy: 'modal-basic-title'}).result.then((result) => {
      ...
      if (this.users.filter(user => user.userId == this.projectModel.managerId).length === 0) {
        ...
        this.projectModel.managerFirstName = undefined;
        this.projectModel.managerLastName = undefined;
  ...
  selectManager() {
    if (!this.selectedManagerId) {
      ...
      this.projectModel.managerFirstName = this.user.firstName;
      this.projectModel.managerLastName = this.user.lastName;
